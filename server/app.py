...
def file_contents(id):
    ...
    skills = resumeparser.extract_information(myString)
    print('in file contents')
    # Call update resume from here
    update_resume(id, email, phone, skills)


def update_resume(id, email, phone, skills):
    resp = applications.find_one({
        '_id': ObjectId(id)
    })
    if resp:
        #skills, 
        resp['Email'] = itemgetter(0)(email)
        resp['Phone'] = itemgetter(0)(phone)
        resp['Skills'] = skills
        applications.update({
            '_id': ObjectId(id)
        },resp)
...
def storeApplication():
    ...
    if new_application:
        ...
        raise InvalidUsage(
            'Something went wrong. Your application was not saved!', status_code=400)
...
def create_opening(request_body):
    ...
    if new_opening:
        ...
        raise InvalidUsage(
            'Something went wrong. Your opening was not saved!', status_code=400)
